...
for sigma in sigmas:
    ...
    for channel in [0, 1, 2]:
        filter_kernel = fspecial_gaussian_2d([filtSize, filtSize], sigma)
        out[..., channel] = convolve2d(noisy[..., channel], filter_kernel, mode='same')  # TODO your code here
    print(skimage.metrics.peak_signal_noise_ratio(out, clean))
    filtered[f'gaussian_{sigma}'] = out

    # Median filter
    out = np.zeros_like(noisy)
    for channel in [0, 1, 2]:
        out[..., channel] = median_filter(noisy[..., channel], size=filtSize)  # TODO your code here
    print(skimage.metrics.peak_signal_noise_ratio(out, clean))
    ...
    print(skimage.metrics.peak_signal_noise_ratio(bilateral, clean))
    ...
    for channel in [0,1,2]:
        nlm[...,channel:channel+1] = nonlocalmeans(noisy[..., channel:channel+1],
                                                   ...
                                                   sigma,
                                                   nlmSigma)
    print(skimage.metrics.peak_signal_noise_ratio(nlm, clean))
...
for r, sigma in enumerate(sigmas):
    ...
    ax[r+1,2].set_title(f'Bilateral filter (sigmaSpatial={sigma}, sigmaInt={sigmaIntensity})')
    ax[r+1,2].axis('off')
